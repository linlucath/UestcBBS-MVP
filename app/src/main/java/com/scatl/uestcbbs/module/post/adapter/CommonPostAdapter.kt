...
import com.scatl.uestcbbs.module.mine.view.MineFragment.is_get_anonymous
...
class CommonPostAdapter(layoutResId: Int, val type: String = "", onPreload: (() -> Unit)? = null) :
    ...
    fun addData(newData: List<CommonPostBean.ListBean>, reload: Boolean) {
        val filterData = newData.filter {
            (if (reload) true else !data.contains(it)) 
            && !BlackListManager.INSTANCE.isBlacked(it.user_id)
            && !(!MineFragment.is_get_anonymous && BlackListManager.INSTANCE.isInAnonymousBoard(it.board_id))
        }
        if (reload) {
